...
#define WINDOW_W		500
#define WINDOW_H		500
#define WINDOW_TITLE	L"HeightMap+Camera"
...
DWORD					g_dwMouseX = 0;
DWORD					g_dwMouseY = 0;
bool bKeyDown_E = false;
bool bKeyDown_Q = false;
...
void ProcessKey( void )
{
	...
	if (GetAsyncKeyState( 'D' )) g_pCamera->MoveLocalX(0.5f);		// 카메라 우로 전진!
	if (GetAsyncKeyState( 'A' )) g_pCamera->MoveLocalX(-0.5f);		// 카메라 좌로 전진!
	
	if (GetAsyncKeyState( 'E' ) || bKeyDown_E)
	{
		if (!bKeyDown_E)
		{
			g_pCamera->RotateLocalZ(-0.5f);	// 카메라 오른쪽 기울기!
			bKeyDown_E = true;
		}
	...
	if (GetAsyncKeyState( 'Q' ) || bKeyDown_Q)
	{
		if (!bKeyDown_Q)
		{
			g_pCamera->RotateLocalZ(0.5f);	// 카메라 왼쪽 기울기!
			bKeyDown_Q = true;
		}
...
INT WINAPI WinMain( HINSTANCE hInst, HINSTANCE, LPSTR, INT )
{
    ...
    // 윈도우 생성
    HWND hWnd = CreateWindow( L"D3D Camera", WINDOW_TITLE,
	...
	delete g_pCamera;
	// 등록된 클래스 소거
    UnregisterClass( L"D3D Camera", wc.hInstance );
    return 0;
}
