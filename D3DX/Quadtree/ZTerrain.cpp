 ...
 */
HRESULT	ZTerrain::Create( LPDIRECT3DDEVICE9 pDev, D3DXVECTOR3* pvfScale, const char* lpBMPFilename, LPCWSTR lpTEXFilename[4] )
{
	m_pd3dDevice = pDev;
	m_vfScale = *pvfScale;
...
HRESULT	ZTerrain::_LoadTextures( LPCWSTR lpTexFilename[4] )
{
	for( int i = 0 ; i < 4 ; i++ )
...
HRESULT	ZTerrain::_BuildHeightMap( const char* lpFilename )
{
	...
	int cxDIB = m_cxDIB;
	int czDIB = m_czDIB;
	if (cxDIB % 2 == 0 || czDIB % 2 == 0) return E_FAIL;
	
	while (cxDIB / 2 != 0 || czDIB / 2 != 0)
	{
		if (cxDIB > 1) cxDIB /= 2;
		if (czDIB > 1) czDIB /= 2;
		if ((cxDIB % 2 == 1 && cxDIB != 1) || (czDIB % 2 == 1 && czDIB != 1)) return E_FAIL;
	}
