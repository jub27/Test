...
ZFLog::ZFLog( UINT32 nTarget, LPCWSTR szFilename )
{
	m_nTarget = nTarget;

	if( nTarget & ZF_LOG_TARGET_FILE )
		lstrcpy( (WCHAR*)m_szFilename, szFilename );
	else
		m_szFilename[0] = NULL;
...
void ZFLog::CreateLogWindow()
{
	...
	m_hwnd = CreateWindow( L"ZLogWindow", ZF_LOG_WINDOW_TITLE, WS_POPUP | WS_CAPTION, x, y, cx, cy, NULL, NULL, GetModuleHandle( NULL ), NULL );
	GetClientRect( m_hwnd, &rc );
	m_hwndList = CreateWindow( L"LISTBOX", L"", WS_CHILD | WS_VSCROLL, 0, 0, rc.right, rc.bottom, m_hwnd, NULL, GetModuleHandle( NULL ), NULL );
