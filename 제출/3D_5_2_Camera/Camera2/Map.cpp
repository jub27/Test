...
LPDIRECT3DVERTEXBUFFER9 g_pBlockVB = NULL;
LPDIRECT3DINDEXBUFFER9	g_pBlockIB = NULL;

LPDIRECT3DTEXTURE9		g_pTexHeight = NULL;
LPDIRECT3DTEXTURE9		g_pTexDiffuse = NULL;
D3DXMATRIXA16			g_matAni;
D3DXMATRIXA16			g_matBlockTm;
D3DXMATRIXA16			g_matBlockRev1;
D3DXMATRIXA16			g_matBlockRev2;
D3DXMATRIXA16			g_matBlockRev3;
...
HRESULT InitVB()
{
    ...
    CUSTOMVERTEX_ vertices[] =
    {
        { -5.0f, 5.0f, 5.0f, 0xffff0000, }, // x, y, z, color
    ...
    if (FAILED(g_pd3dDevice->CreateVertexBuffer(8 * sizeof(CUSTOMVERTEX_), 0, D3DFVF_CUSTOMVERTEX,
    ...
    if (FAILED(g_pBlockVB->Lock(0, sizeof(vertices), (void**)&pVertices, 0)))
    {
        return E_FAIL;
    }
...
HRESULT InitIB()
{
    ...
    if (FAILED(g_pd3dDevice->CreateIndexBuffer(12 * sizeof(MYINDEX), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &g_pBlockIB, NULL))) {
        return E_FAIL;
    }

    if (FAILED(g_pBlockIB->Lock(0, sizeof(indices), (void**)&pIndices, 0))) {
        return E_FAIL;
    }
...
void ProcessKey(void)
{
    ...
    if (GetAsyncKeyState(VK_UP)) g_pCamera->MoveLocalY(0.5f);
...
//	D3DXMatrixRotationY( &g_matAni, d / 1000.0f );
    ...
    D3DXMatrixIdentity(&g_matBlockRev1);
    D3DXMatrixIdentity(&g_matBlockRev2);
    D3DXMatrixIdentity(&g_matBlockRev3);
    ...
    D3DXVECTOR3 normal(1, 0, 0);

    D3DXMatrixTranslation(&g_matBlockTm, 0, -10, 50);
    D3DXMatrixRotationAxis(&g_matBlockRev1, &normal, g_pCamera->GetRotateY());
    //D3DXMatrixRotationAxis(&g_matBlockRev2, g_pCamera->GetView(), -g_pCamera->GetRotateZ());
    D3DXMatrixRotationAxis(&g_matBlockRev3, g_pCamera->GetUp(), g_pCamera->GetRotateX());
...
void DrawMesh(D3DXMATRIXA16* pMat)
{
    ...
    g_pd3dDevice->SetTransform(D3DTS_WORLD, &(g_matBlockTm * g_matBlockRev1 * g_matBlockRev2 * g_matBlockRev3 * g_matCameraTm));
