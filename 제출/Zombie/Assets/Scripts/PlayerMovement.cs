...
public class PlayerMovement : MonoBehaviour {
    ...
    private void FixedUpdate() {
        ...
        playerAnimator.SetFloat("Move",Mathf.Abs(playerInput.moveH) + Mathf.Abs(playerInput.moveV));
    }

    // 입력값에 따라 캐릭터를 앞뒤로 움직임
    private void Move() {
        // 상대적으로 이동할 거리 계산
        Vector3 dir = followCam.transform.forward * playerInput.moveV + followCam.transform.right * playerInput.moveH;
        dir.y = 0;
        dir.Normalize();
    ...
    private void Rotate() {
        ...
        gameObject.transform.forward = (mousePoint - gameObject.transform.position).normalized;
    }
}