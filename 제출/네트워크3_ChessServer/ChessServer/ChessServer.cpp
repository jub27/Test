	...
	while (1)
	{
		...
		if (ptr->recvbytes == 0)
		{
			...
			case REQUEST_PLAYER_NUMBER:
				cout << "!" << endl;
				ptr->buf[0] = ASSIGN_PLAYER_NUMBER;
				ptr->buf[1] = nTotalSockets-1;
				break;
			case REQUEST_GAME_START:
				cout << "!!" << endl;
				ptr->buf[0] = GAME_START;
				break;
			case PIECE_MOVE:
				cout << "!!!" << endl;
				break;
		...
		if (ptr->recvbytes > ptr->sendbytes)
		{
			...
			if (ptr->buf[0] == GAME_START || ptr->buf[0] == WHITE_WIN || ptr->buf[0] == BLACK_WIN) {
				if (nTotalSockets == 3) {
					for (int i = 1; i < nTotalSockets; i++) {
						retval = WSASend(SocketInfoArray[i]->sock, &ptr->wsabuf, 1, &sendbytes, 0, &ptr->overlapped, NULL);
					}
				}
			}
			else if (ptr->buf[0] == PIECE_MOVE) {
				if(index == 1)
					retval = WSASend(SocketInfoArray[2]->sock, &ptr->wsabuf, 1, &sendbytes, 0, &ptr->overlapped, NULL);
				else
					retval = WSASend(SocketInfoArray[1]->sock, &ptr->wsabuf, 1, &sendbytes, 0, &ptr->overlapped, NULL);
			}
			else {
